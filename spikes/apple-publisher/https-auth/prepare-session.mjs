...
const entryPath = process.argv.includes('--replacement-sdk') ? '/replacement-sdk' : process.argv.includes('--replacement-direct') ? '/replacement-direct' : process.argv.includes('--same-id') ? '/same-id' : process.argv.includes('--direct') ? '/control' : '/';
...
const explanation = entryPath === '/same-id'
  ? 'The same-ID proof authorizes first. Each separately labeled button performs one bounded disposable playlist operation.'
  : entryPath.startsWith('/replacement-')
    ? `The ${entryPath.endsWith('direct') ? 'direct fallback' : 'SDK'} proof authorizes first, creates one fresh disposable playlist, then exposes one separate replacement button.`
    : 'The test only authorizes Apple Music and reads your storefront. It does not publish playlists.';
await writeFile(new URL('https-invitation.html', directory), `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="referrer" content="no-referrer"><title>Protected Apple Music test invitation</title></head><body><h1>Protected Apple Music test</h1><p>This invitation expires at ${new Date((now + 840) * 1000).toISOString()}.</p><a href="${origin}${entryPath}#invite=${invite}">Open the HTTPS authorization test</a><p>${explanation}</p></body></html>`, { mode: 0o600 });
