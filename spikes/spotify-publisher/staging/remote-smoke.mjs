...
if (process.argv.includes("--exercise-invitation")) {
  ...
  const postOptions = { method: "POST", headers: { Origin: origin, Cookie: inviteCookie, "Content-Type": "application/x-www-form-urlencoded" }, body: form };
...
console.log(JSON.stringify({ checkedAt: new Date().toISOString(), origin, configuredCallback: origin + "/auth/callback", publisherAuthorized: state.authorized, providerWritesPerformed: false, checks: results }, null, 2));
