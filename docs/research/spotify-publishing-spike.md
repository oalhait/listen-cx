...
| Staging Worker-runtime contracts | 21 passing tests with simulated provider responses and real workerd request validation |
...
The readback endpoint independently fetches metadata, contents, and metadata again, accepting only an unchanged snapshot. It can report drift even when the stored revision is marked applied, without repairing that drift. Rate-limit deadlines survive restart. All 49 Node/core/harness/runner fixture tests and 21 Worker fixture tests passed; both spike typechecks and the staging dry run passed. After the September 12 transport deployment, thirteen read-only/protected-negative HTTPS checks passed and the protected credential-free token probe reached Spotify with HTTP 405. No real playlist write or authenticated playlist readback has yet occurred.
