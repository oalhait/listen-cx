...
The isolated HTTPS staging publisher is deployed and authorized as `mostar123`; local and staging contract tests pass. **Authenticated creation plus exact same-ID `[A,B,C]` to `[C,A,D]` readback are verified.** The separate-listener experiment is deferred because Omar has no second Spotify account and does not require that proof before Threads work. The website remains authoritative; this spike creates one public publisher-owned playlist per opaque website playlist key.
...
| Actual app mode, Premium and publisher allowlist | `mostar123` and Development mode confirmed by Omar; app-owner Premium was not independently inspected |
| Staging Worker-runtime contracts | 22 passing tests with simulated provider responses and real workerd request validation |
| Actual staging HTTPS security checks | 14 read-only/protected-negative checks pass on the current deployment |
| Authenticated Spotify create/update and readback | `[A,B,C]` creation and `[C,A,D]` same-ID update both verified by exact provider readback |
| Listener outside OAuth allowlist saving and observing edits inside Spotify | Deferred by user; no separate Spotify account is available or required for the Threads milestone |

Initially no Spotify credentials were available. Omar subsequently added `SPOTIFY_CLIENT_ID` to Doppler `listen-cx/dev_personal`; a non-logging subprocess confirmed its presence. The first OAuth attempt failed on dashboard redirect matching. After the callback was corrected, consent reached token exchange, but the subsequent `/me` request failed. The first harness discarded the original status/body; a subsequent attempt preserved the actual 403 allowlist rejection, documented below. On September 12, Omar rotated to his brother's new app, confirmed `mostar123` and Development mode, and completed the live create/update proof. No production deployment or existing personal playlist mutation occurred.

Test-first evidence: initial publisher and harness tests failed because their implementation modules did not exist. After implementation, 16 publisher and 7 HTTP tests passed. The recovery contract then failed with `recoverCreate is not a function`; its HTTP test failed with expected 409 versus actual 404. Both passed after implementation. The discovered-identity setup test then failed with `missing_configuration`; it passed after adding explicit confirmation, including rejection of unconfirmed writes. After the live OAuth failure exposed discarded `/me` diagnostics, five more tests failed against the old status/error mapping and passed after preserving status and redacted provider fields. The current checkpoint has 49 Node tests plus 21 Worker tests. The readback tests also cover changing snapshots, missing items, and relinked track IDs without claiming equivalence.
...
| `GET /control/token-transport` | Operator bearer; credential-free token-endpoint reachability and status only |
| `GET /control/publisher-transport` | Operator bearer; confirmed-token `/me` reachability and publisher-match boolean only |
...
Twenty-two Worker-runtime tests pass with simulated provider responses. They cover operator authentication, missing-secret failure, HTTPS/host/origin constraints, body bounds, preview-safe invitation POST, cookie/state/PKCE binding, replay/expiry, concurrent callbacks, encrypted and tampered storage, explicit confirmation, refresh-token rotation, serialized controls, durable instance restart with stable playlist reconciliation, path-aware browser/operator host isolation, edge-runtime request construction, redirect rejection, bounded transport classification and native fetch receiver binding. The origin-split regression first failed against the old workers.dev invitation and permissive same-host public endpoints, then passed after the change. Initial contract tests failed against the placeholder Worker; specific security regressions were observed red before fixes. An independent security review of the Worker, config and core publisher found no blocking isolation issue and requested the preview-safe landing flow, which was added and tested before exposure.

The first consent attempt after the September 12 Client ID rotation reached the callback but returned `authorization_unavailable`. The installed workerd reproduced the exact cause: `redirect: "error"` is rejected while constructing the token request, before any Spotify subrequest is sent. Provider calls now use `manual` and explicitly reject every 3xx response without following it or exposing `Location`. The Worker persists only the failed phase, a bounded kind, optional response status and observation time; it never stores the thrown message, OAuth code, PKCE verifier, token, headers or response body. Tests now pass each provider request through workerd's real `Request` constructor before the provider mock, so the unsupported option cannot be hidden again.

After deploying version `96735d92-b468-4ca0-8e81-adae4c0cbdc4`, thirteen read-only/protected-negative HTTPS checks passed at `2026-09-12T20:43:09.937Z`. The authenticated credential-free transport probe reached Spotify and received HTTP 405, proving staging egress and request construction without using OAuth material. Protected status remained unauthorized with no publisher, candidate, destination, verification failure or retained authorization failure. No provider playlist request or write occurred.

The first live publish then exposed a second workerd-specific failure before playlist creation. `SpotifyPublisher` stored native `fetch` and invoked it as an instance method, so workerd rejected the incorrect receiver with `Illegal invocation`. Durable state showed `providerPlaylistId=null` and `createUnresolved=false`; the failed receipt was preserved and no alternate key was attempted. The default fetcher now uses a lexical wrapper, while injected test fetchers keep their original semantics. A receiver-sensitive Worker regression failed three publication tests before the fix and passed after it. On version `d1616c4d-ff1c-49a3-bff2-cdcb300be214`, the protected default-fetch probe reached `/v1/me` and matched the confirmed publisher before the same-key retry.

Actual HTTPS smoke passed twenty checks at `2026-09-10T05:34:38Z`: health; unauthorized reads/writes; cross-origin controls; invalid callback/invitation; operator status; expendable invitation creation; repeated preview GET; malicious-origin POST; valid browser-bound redirect; invitation replay; missing browser; denied-consent consumption; callback replay; rejection of OAuth on the operator host; and 404 responses for all three wildcard-matched path suffixes. The smoke never followed the redirect to Spotify and made no provider writes. It consumed its expendable invitation. Before/after status and SHA-256 checks confirmed the staging homepage and `/control/status` response were unchanged; the latter remains a main-site 404. A separate fresh friend invitation was issued only after these checks passed. Authenticated provider create/update/readback subsequently passed; the second-account native Spotify test was deferred.
...
`spikes/spotify-publisher/live-test.ts` captures fresh provider readback after each phase. The initial phase publishes `[A,B,C]` at revision 1. The updated phase publishes `[C,A,D]` at revision 2 only when the stored destination still matches the initial receipt's playlist ID. Each receipt records observed track URIs, owner ID, snapshot ID and observation time. API verification always leaves `listenerVerification: pending`; a successful API receipt does not prove native-library propagation. The original plan paused between phases for a separate listener, but Omar explicitly deferred that optional proof before running revision 2.

The initial live phase succeeded on September 12 at `2026-09-12T20:58:16.757Z`. Spotify returned playlist [`2U5fOAZV7CTG1nd9M9XapM`](https://open.spotify.com/playlist/2U5fOAZV7CTG1nd9M9XapM), owned by `mostar123`, with revision and applied revision 1, snapshot `AAAAAZkaPElFF1o8wepYADXwaXBNWCs2`, and exact ordered URIs for A, B and C. Fresh readback reported `matchesDesired=true`. Omar observed the expected initial playlist and owner-account state; this was not an independent listener test.

The controlled update succeeded at `2026-09-12T21:14:30.926Z` on the same playlist ID and owner. Revision and applied revision advanced to 2, the snapshot changed to `AAAAAsChi/yKLk39OkSGzK1dh4YKVsO8`, and exact readback returned C, A and D with `matchesDesired=true`. This proves reorder, preservation, removal and addition through the Spotify API without creating a second playlist. `listenerVerification` remains `pending` in both receipts to keep the deferred native-listener claim explicit.
...
The readback endpoint independently fetches metadata, contents, and metadata again, accepting only an unchanged snapshot. It can report drift even when the stored revision is marked applied, without repairing that drift. Rate-limit deadlines survive restart. All 49 Node/core/harness/runner fixture tests and 22 Worker fixture tests passed; both spike typechecks and the staging dry run passed. Fourteen read-only/protected-negative HTTPS checks passed on the current deployment; the protected token probe and default-fetch publisher probe both reached Spotify. Authenticated create, removal, addition, reorder and same-ID readback are verified. Independent listener save/propagation is explicitly deferred.

## Spotify parity with the Apple web authorization milestone

**Subsequent September 12 clarification:** Omar wants one shared provider playlist per Thread, saved by every listener. The per-user-copy recommendation below was based on an earlier interpretation and is not required for that topology. The existing shared publisher is the relevant starting point. Listeners who only save the public playlist inside Spotify do not need our OAuth connection. Shared-publisher create/update/readback are verified; independent saved-listener propagation is deferred.

For the completed development-app authorization, the app owner used Spotify Developer Dashboard → app → Settings → Users Management to authorize the intended publisher. Omar confirmed the actual App Status as Development mode and the callback remains `https://staging.listen.cx/auth/callback`. The earlier “The user is not registered for this application” 403 came from a different account absent from the prior app's allowlist; adding OAuth scopes would not have repaired it. [Current setup instructions and five-user development limit](https://developer.spotify.com/documentation/web-api/concepts/quota-modes).
...
| --- | --- | --- |
| Web OAuth and authenticated identity | PKCE, HTTPS callback, state/browser binding, encrypted tokens, `/me`, explicit operator confirmation | Successful publisher `/me`, encrypted consent persistence and exact operator confirmation verified. No website user-connection flow exists. |
| User-specific catalog context | Publisher token is used for provider reads; source metadata clients are separate | Authenticated owner identity and playlist reads are verified. The publisher country was not captured; Spotify uses the token user's country for playlist-item availability. |
