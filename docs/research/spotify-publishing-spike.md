...
The isolated HTTPS staging publisher is deployed and authorized as `mostar123`; local and staging contract tests pass. **Initial authenticated Spotify creation and exact readback are verified; the update phase and second-account listener experiment remain pending.** The website remains authoritative; this spike creates one public publisher-owned playlist per opaque website playlist key. Listeners save that shared playlist inside Spotify. Their accounts never authorize this harness.
...
| Authenticated Spotify create/update and readback | Initial `[A,B,C]` creation/readback verified; `[C,A,D]` update deliberately pending native listener baseline |
...
The initial live phase succeeded on September 12 at `2026-09-12T20:58:16.757Z`. Spotify returned playlist [`2U5fOAZV7CTG1nd9M9XapM`](https://open.spotify.com/playlist/2U5fOAZV7CTG1nd9M9XapM), owned by `mostar123`, with revision and applied revision 1, snapshot `AAAAAZkaPElFF1o8wepYADXwaXBNWCs2`, and exact ordered URIs for A, B and C. Fresh readback reported `matchesDesired=true`. The update phase has not run; `listenerVerification` remains pending until a separately identified account outside the app allowlist saves and inspects the playlist in native Spotify.
...
The readback endpoint independently fetches metadata, contents, and metadata again, accepting only an unchanged snapshot. It can report drift even when the stored revision is marked applied, without repairing that drift. Rate-limit deadlines survive restart. All 49 Node/core/harness/runner fixture tests and 22 Worker fixture tests passed; both spike typechecks and the staging dry run passed. Fourteen read-only/protected-negative HTTPS checks passed on the current deployment; the protected token probe and default-fetch publisher probe both reached Spotify. Initial authenticated creation and readback are now verified; native listener propagation and the same-ID update remain pending.
