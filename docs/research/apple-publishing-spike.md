...
Research checked September 12, 2026 (UTC); latest live playlist proof is September 12. Branch: `omar/apple-publishing-spike`. This decision supersedes the native publisher investigation below. **The product is web-only:** people collaborate on an ordered Thread on our website, then listen in their existing Spotify or Apple Music app. No companion app, device provisioning, or native publisher runtime is part of the proposed product.

## Recommendation

**Latest clarification, September 12: use one shared Apple playlist per Thread.** Omar intends every Apple listener to save the same destination, rather than receiving an independently owned copy. The per-user destination discussion below describes an earlier interpretation and is not the current recommendation. Listener propagation and publisher access are separate: Apple documents that edits to shared playlists appear on followers' devices, while our service still needs an authorized writer for the single destination. See the new evidence section below.

**Do not promise full Apple Music sync yet.** The September 12 live proof verified creation and append on one public playlist ID, but the undocumented same-ID replacement request failed before removal could be verified. The public Apple web/server documentation exposes no operation that removes tracks or replaces their order on an existing playlist. A stable provider ID and public link across `[A,B,C]` → `[C,A,D]` therefore have no verified full-sync implementation using our web credentials. The unclassified browser failure does not establish an Apple HTTP rejection and leaves one narrow transport-observed test open. [Apple playlist API](https://developer.apple.com/documentation/applemusicapi/playlists-api)

**Recreating the entire playlist per revision is technically supported and has an isolated experiment prepared.** A website pointer could select the newest verified playlist for fresh opens. This changes provider identity; it does not update an already-saved Apple reference. Product adoption remains undecided. Keep the website's ordered revision authoritative and continue the separate Spotify route. The broader partner search closes the strongest apparent lead: TuneMyMusic's current compatibility table explicitly limits Apple sync to **Add Only**, contradicting its older blog's removal claim. MusicAPI.com's enterprise API also explicitly lacks the required Apple edits. Apple publishing remains blocked under the current product scope; playlist export is not being restored. The copy/export alternatives below are research comparisons that would require a separate product decision. A verified undocumented operation or separate commercial agreement could reopen full sync with evidence of capabilities beyond these published restrictions. [TuneMyMusic compatibility](https://www.tunemymusic.com/features/supported), [MusicAPI compatibility](https://musicapi.com/docs/api-basics/supported-features/)
...
## Shared destination and open-source mutation leads, September 12

The absence of a documented public REST operation is not proof that playlist removal or reordering is technically impossible. Current research distinguishes three surfaces:

| Surface | Evidence | Implication |
| --- | --- | --- |
| Public Apple Music REST API | The playlist collection lists create and append under mutation operations; append explicitly adds at the end. No removal, clear, or reorder contract was found. [Collection](https://developer.apple.com/documentation/applemusicapi/playlists-api), [append](https://developer.apple.com/documentation/applemusicapi/add-tracks-to-a-library-playlist) | Our normal developer-token integration has no documented full replacement contract. |
| Native MusicKit | `MusicLibrary.edit(...items:)` rebuilds entries of a playlist created by the app on the listed iOS-family platforms. The current web declaration includes Mac Catalyst, while the installed iPhoneOS 26.2 SDK marks Mac Catalyst unavailable; it also errors for a playlist created by another app. [MusicLibrary edit](https://developer.apple.com/documentation/musickit/musiclibrary/edit(_:name:description:authordisplayname:items:)) | Apple has an official native capability, but this does not supply a web/server endpoint and the native product path remains stopped. |
| Undocumented web operations | Cider and applemusic-mcp contain additional mutation calls, detailed below. | There are concrete implementation leads worth isolated verification; their source does not prove current acceptance with our credentials. |

Cider's archived open-source implementation connects drag completion to `put()`, which sends the entire ordered `{id,type}` list using `PUT /v1/me/library/playlists/{id}/tracks` through `mk.api.v3.music`. Its remove handler edits the local list and calls the same PUT. The code dates to August 2022 and is not evidence from a normal website: Cider runs Electron with `webSecurity: false`, rewrites Apple request `Origin` and `Referer` to `https://beta.music.apple.com`, and obtains its developer token from `api.cider.sh`. It therefore proves a concrete first-party-style request shape, not current acceptance with an ordinary app's developer token and Music User Token. [Pinned replacement/removal implementation](https://github.com/ciderapp/Cider/blob/953bb5a9e26080bba2bb20438f0ddf38b0a8b260/src/renderer/views/pages/playlist-inline.ejs#L467-L507), [Electron security setting](https://github.com/ciderapp/Cider/blob/953bb5a9e26080bba2bb20438f0ddf38b0a8b260/src/main/base/browserwindow.ts#L331-L345), [Apple header rewriting](https://github.com/ciderapp/Cider/blob/953bb5a9e26080bba2bb20438f0ddf38b0a8b260/src/main/base/browserwindow.ts#L713-L730), [token source](https://github.com/ciderapp/Cider/blob/953bb5a9e26080bba2bb20438f0ddf38b0a8b260/src/renderer/index.js#L59-L114).

Parachord is stronger and current ordinary-credential evidence. Its August 2026 code builds `Authorization: Bearer <developer token>` plus `Music-User-Token` from its own app configuration; its build now [mints that developer token from its own `.p8`](https://github.com/Parachord/parachord-mobile/commit/b055b9469618a453ff994a45bc19a1fcffe94fd8). The client makes a direct Ktor request to `https://api.music.apple.com/v1/me/library/playlists/{id}/tracks`. It probes `PUT`, treats 401/403/405 as unsupported for that session, and falls back to documented `POST` append. The provider currently advertises replacement and removal as unsupported. No successful PUT is recorded in the current repository. Its June issue 272 captured a real `40007` malformed-body response from the POST fallback at the cited stack line; requiring `type: "songs"` fixed append serialization, not replacement. [App credential assembly](https://github.com/Parachord/parachord-mobile/blob/4b42e3248909fefe48c22b367dbd10b0aa9e5dbf/app/src/main/java/com/parachord/android/data/auth/AndroidAuthTokenProvider.kt#L27-L34), [direct PUT and POST client](https://github.com/Parachord/parachord-mobile/blob/4b42e3248909fefe48c22b367dbd10b0aa9e5dbf/shared/src/commonMain/kotlin/com/parachord/shared/api/AppleMusicLibraryClient.kt#L145-L170), [current degradation logic](https://github.com/Parachord/parachord-mobile/blob/4b42e3248909fefe48c22b367dbd10b0aa9e5dbf/shared/src/commonMain/kotlin/com/parachord/shared/sync/AppleMusicSyncProvider.kt#L358-L403), [issue 272 diagnosis](https://github.com/Parachord/parachord-mobile/issues/272#issuecomment-4791679160).

### Live same-ID mutation result, September 12

The protected MusicKit JS proof created `[704790294,6782695839,617154366]` and then appended `202272624` on library playlist `p.ldvALDXu8v2g0Yz`. Complete API readback verified `[704790294,6782695839,617154366,202272624]` on that same library ID. Apple returned public catalog ID `pl.u-8aAVz46foR1g69A` and a public Apple Music URL for the same destination. A fresh logged-out fetch of that public page contained all four catalog IDs.

The next operation durably reserved and submitted the Cider-derived `PUT /v1/me/library/playlists/p.ldvALDXu8v2g0Yz/tracks` with the retained library-song entries for `[704790294,617154366,202272624]`. MusicKit rejected the call before the harness could classify an HTTP response, leaving `UNKNOWN_ERROR` with no HTTP status. The last trusted API readback and the public page both remained at all four songs, including `6782695839`. Removal therefore did not verify. The journal keeps that operation submitted, and the sequential fence correctly prevented the reorder attempt.

The failure does not establish whether Apple rejected the HTTP method or caller. Current MusicKit v3 still accepts `music(path, query, {fetchOptions})`; its fetch middleware turns a non-2xx response into an error whose `data` is the HTTP `Response`, and the harness already reads `error.data.status`. No status therefore means the attempt failed before a response was exposed to the page or outside that normal response path. A separate read-only `OPTIONS` request on September 12 returned `204` and allowed the spike origin, the requested authorization headers, and `GET,HEAD,PUT,PATCH,POST,DELETE,OPTIONS`. That tested preflight succeeded; the failed browser attempt's transport remains unclassified. This does not prove the underlying PUT is accepted. The request shape matched Cider's retained library-item `{id,type}` list. No blind retry, alternate hostname, web-player credential, private endpoint, deletion request, or playlist recreation followed. [Current MusicKit JS](https://js-cdn.music.apple.com/musickit/v3/musickit.js)

The smallest decisive web test is a fresh disposable playlist, not a retry of the fenced operation. First read and record `canEdit`, `isPublic`, and `hasCatalog`. Install a passive fetch observer before MusicKit loads, then submit one replacement and capture only whether the exact PUT was issued, whether fetch produced a response, its status, and an allowlisted Apple error code. If the SDK fails before issuing or exposing the request, use a separate disposable playlist for one direct browser `fetch` with the same app's developer token and the fresh Music User Token returned by normal MusicKit authorization. Reconcile exact same-ID readback after every uncertain result. This distinguishes wrapper behavior, transport failure, credential rejection, and body rejection without web-player credentials or AMP impersonation.

### Fresh instrumented replacement proof, prepared September 12

Commit `aea33be184f685db6209e6776307ecf7000e830f` implements that bounded test on separate `/replacement-sdk` and `/replacement-direct` pages. The SDK path creates one new disposable `[A,B,C]` destination, verifies its complete order and `canEdit`, `isPublic`, and `hasCatalog` metadata, then submits exactly one replacement to `[C,A]`. A fetch observer installed before MusicKit loads records only the exact method and path, whether the PUT was issued, whether fetch exposed a response, its HTTP status, and an allowlisted Apple error code. It forwards the original receiver, arguments, and promise unchanged. Transport evidence is persisted before readback, and the journal remains submitted unless complete same-ID readback matches `[C,A]`.

The direct page uses a different active journal and a different disposable playlist. It stays locked unless the SDK journal has a complete unchanged `[A,B,C]` readback and no exposed HTTP response. An issued HTTP response of any status, a failed or incomplete readback, a changed destination, or a verified SDK replacement keeps the fallback locked. If eligible, its one PUT uses this app's short-lived developer JWT and the fresh Music User Token returned by normal MusicKit authorization in memory; neither token is persisted or rendered.

Validation passed 55 focused Node tests, nine Workers-runtime tests, the affected TypeScript check, syntax and diff checks, an exact 15-asset build, and the isolated deployment dry run. Tests cover an SDK failure before fetch, a rejected issued fetch, a captured `403`/`40300`, readback failure after that response, a successful observed `204`, late observer settlement, the direct eligibility gate, separate destination identity, and durable submission before writes. Dev deployment `8f83dc93-220b-47b2-ac32-0f17bc9e500c` passed the remote isolation and checked-source smoke test at 2026-09-12 21:53:35 UTC: landing `200`, unauthenticated token `401`, foreign session `403`, unknown control `404`, protected session `204`, protected token `200`, and `no-store` token caching.

Omar ran the SDK page in a normal browser. At 2026-09-12 21:59:05.565 UTC, complete API readback verified new library playlist `p.eoGxL6oTkgopd54` at `[704790294,6782695839,617154366]`. Apple reported `canEdit=true`, `isPublic=true`, `hasCatalog=true`, catalog ID `pl.u-WabZzGahd3Kj9m7`, and [this public URL](https://music.apple.com/us/playlist/disposable-listen-cx-replacement-sdk-sdk-3918f269-9da9/pl.u-WabZzGahd3Kj9m7). The retained library-song identities were `i.1YBm2oGUqYZg618`, `i.1YBxX6VsqYZg618`, and `i.AWPENLBsLdm2VXQ` in that order.

At 21:59:06.250 UTC, the `[C,A]` replacement was durably reserved and submitted. The observer recorded the exact `PUT /v1/me/library/playlists/p.eoGxL6oTkgopd54/tracks` with `issued=true`, `responseExposed=false`, and no HTTP status or Apple error code. The operation remained submitted. Its mandatory complete readback and a subsequent user-initiated complete read both returned the unchanged `[A,B,C]` sequence on the same ID with the same library-song identities. This meets the direct fallback gate. It does not show an Apple HTTP rejection because no response was exposed. The SDK PUT was not retried, the earlier uncertain playlist remains untouched, and the separate direct destination was pending when this evidence was recorded.

Omar then ran the eligible direct page in the same browser profile. It created fresh library playlist `p.GE5rLDDSPXZB7aE` and verified `[A,B,C]` at 2026-09-12 22:02:30.054 UTC. Apple again reported `canEdit=true`, `isPublic=true`, and `hasCatalog=true`, with catalog ID `pl.u-NpXmDggFmMdP6Bq` and [this API-returned public URL](https://music.apple.com/us/playlist/disposable-listen-cx-replacement-direct-direct-0ffb5423/pl.u-NpXmDggFmMdP6Bq). The three catalog-to-library mappings matched the SDK destination's library-song identities.

At 22:02:30.634 UTC, the direct `[C,A]` replacement was durably reserved and submitted. The direct browser fetch recorded the exact `PUT /v1/me/library/playlists/p.GE5rLDDSPXZB7aE/tracks` with `issued=true`, `responseExposed=false`, and no HTTP status or Apple error code. This rules out MusicKit's `api.music` wrapper as the sole cause of the no-response behavior. It still does not classify an Apple HTTP result or prove that Apple rejects replacement. The pasted snapshot had no replacement `readback` field; its displayed `lastReadback` was still the creation verification at `[A,B,C]`. Therefore, it did not yet prove post-PUT state. The direct write remains submitted, and no retry, replacement destination, or additional write is allowed. A separate read-only readback was requested on the existing page.

At approximately 22:04 UTC, an independent public catalog API read used only the app's developer token. Both the SDK catalog resource `pl.u-WabZzGahd3Kj9m7` and direct catalog resource `pl.u-NpXmDggFmMdP6Bq` returned HTTP `200`, the requested resource ID, complete `[704790294,6782695839,617154366]` tracks, and no next page. This verifies that both public projections remained `[A,B,C]` after the direct attempt. It does not replace the missing personalized library readback for the submitted direct operation.

The in-app browser had loaded the protected SDK page but did not surface Apple's authorization popup. The Chrome extension exposed unrelated existing tabs, but the controlled Chrome session/open action timed out twice and could not claim Omar's live test tab; no shell or alternate GUI automation followed. The browser-control handoff is separate from the SDK transport result above.

The MIT-licensed applemusic-mcp implements per-track removal using `DELETE /v1/me/library/playlists/{id}/tracks` with library-track IDs and `mode=all` on `amp-api.music.apple.com`. Its current `_headers()` deliberately selects an Apple web-player token and its own comments report rejection of generated developer tokens on that host. Thus it is not evidence that changing the hostname in our existing authorized MusicKit integration enables removal. Its README distinguishes documented developer-token operations from the web-session path. This assessment inspected source only; no package was installed, browser credentials extracted, or provider mutation attempted. [Pinned removal implementation](https://github.com/epheterson/applemusic-mcp/blob/ca94ca2630cfd721ecf3ff008637c84ae60b0ea0/src/applemusic_mcp/amp_api.py#L409-L424), [credential selection](https://github.com/epheterson/applemusic-mcp/blob/ca94ca2630cfd721ecf3ff008637c84ae60b0ea0/src/applemusic_mcp/amp_api.py#L118-L127), [project](https://github.com/epheterson/applemusic-mcp).

An open-source adapter could add ordered reconciliation, complete pagination, durable receipts, capability reporting, and tested wrappers for verified operations. It cannot create an Apple server capability merely by exposing a new local method. The open question is whether the replacement operation accepts our own normally authorized credentials and preserves the destination and public URL when the transport result is observed precisely. A path requiring Apple web-player credentials or first-party request impersonation remains outside the product boundary.

Apple's current sharing guide says edits to shared playlists are reflected on followers' devices. Its collaborative-playlist guide separately allows participants to add, remove, and reorder songs in Apple Music. For Threads, a shared playlist with one controlled publisher better preserves website authority; native collaboration is not required simply to distribute updates and would introduce another editing surface. [Shared-playlist propagation](https://support.apple.com/guide/iphone/share-music-with-friends-iphe5a418a82/ios), [native collaboration](https://support.apple.com/en-us/118494).

The revised acceptance test uses one publisher-owned destination and a second independent listener who saves its shared reference. Verify initial contents, update the same destination after the listener closes the website, then inspect the listener's saved playlist and the publisher's complete API readback. Test same-ID removal/reordering separately against the identified replacement lead. A successful publisher write does not establish follower propagation latency; a follower seeing edits does not establish unattended publisher authorization. The existing Thread/provider publication key fits this shared topology; job scheduling, private publisher credentials, and a verified adapter remain necessary.

## Thread additions to a connected Apple library, September 12

**Apple documents the operations needed for an additions-only mirror: create a playlist in the consenting user's library, then append songs to that same playlist.** The September 12 foreground proof verified both operations and exact same-ID readback. It does not establish updates while the owner is away, append replay behavior, or follower propagation. The two-playlist recreation experiment remains a separate option for non-append revisions; it is not required merely to add songs.
...
The remaining backend proof should reuse the verified public destination shape: close the owner browser, add a song through the Thread, and verify the same Apple ID through complete API readback and the Music app. Replay the same Thread request and verify that no second append occurs. The foreground proof already implements pagination and reservation-before-write receipts; those controls must carry into the backend adapter. Removal and reorder stay blocked rather than falling back to recreation under the shared-playlist product decision.
...
`spikes/apple-publisher/https-auth/` began as an isolated authorization-only Worker at [the dev probe host](https://listen-cx-apple-auth-spike-dev.omar-alhait.workers.dev). It used the same developer signing credentials, 15-minute token claims, MusicKit v3 distribution, MusicKit app configuration, and CSP/referrer policy as the local harness. That experiment changed its HTTPS origin and browser session; the later referrer-policy correction is described below. The protected origin later gained separate direct-control and disposable same-ID proof pages; it still has no provider proxy.
...
Apple's [Music web guide](https://support.apple.com/guide/music-web-review/create-edit-and-delete-playlists-apdmc7b0b200/web) shows that its first-party website can remove songs from a playlist. That proves a server capability exists somewhere, not that ordinary developer credentials may invoke it. In the long-running [DELETE/PUT Apple Music API thread](https://developer.apple.com/forums/thread/107807), Apple engineers repeatedly described the public surface as adding to libraries and editable playlists, requested enhancement feedback for deletion, and distinguished the private `amp-api` surface used by Apple products. The newest January 2026 post is another developer request, not a new Apple API announcement.
...
An October 2025 [DTS response about deleting playlist songs](https://developer.apple.com/forums/thread/805461) first asks the developer to inspect `LibraryPlaylists.Attributes.canEdit`. The developer then reports `canEdit=true` and still no successful deletion; Apple provides no follow-up endpoint. Treat `canEdit` as a required precondition and useful diagnostic, not proof that a DELETE or PUT contract exists.
