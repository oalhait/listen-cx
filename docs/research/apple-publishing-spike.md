...
**Do not promise full Apple Music sync.** The September 12 live proof verified creation and append on one public playlist ID, but the undocumented same-ID replacement request failed before removal could be verified. The public Apple web/server documentation exposes no operation that removes tracks or replaces their order on an existing playlist. A stable provider ID and public link across `[A,B,C]` → `[C,A,D]` therefore have no verified full-sync implementation using our web credentials. This is a conclusion about the reviewed supported surfaces and the tested Cider-derived lead, not proof that no private commercial agreement could provide one. [Apple playlist API](https://developer.apple.com/documentation/applemusicapi/playlists-api)
...
| Native MusicKit | `MusicLibrary.edit(...items:)` rebuilds entries of a playlist created by the app. [MusicLibrary](https://developer.apple.com/documentation/musickit/musiclibrary) | Apple has an official native capability, but this does not supply a web/server endpoint and the native product path remains stopped. |
...
Cider's archived open-source implementation connects drag completion to `put()`, which sends the entire ordered `{id,type}` list using `PUT /v1/me/library/playlists/{id}/tracks` through `mk.api.v3.music`. Its remove handler edits the local list and calls the same PUT. This is a direct same-ID replacement lead, not playlist recreation. The repository was archived in December 2024; no current runtime success or compatibility with our developer credentials was established. [Pinned drag handler](https://github.com/ciderapp/Cider/blob/953bb5a9e26080bba2bb20438f0ddf38b0a8b260/src/renderer/views/pages/playlist-inline.ejs#L165-L167), [pinned replacement/removal implementation](https://github.com/ciderapp/Cider/blob/953bb5a9e26080bba2bb20438f0ddf38b0a8b260/src/renderer/views/pages/playlist-inline.ejs#L467-L507).
...
The failure does not establish whether Apple rejected the HTTP method, rejected the caller, or failed inside MusicKit's generic request layer. It does establish that the archived route is not a usable removal contract in the current integration. The request shape matched the pinned Cider source. No blind retry, alternate hostname, web-player credential, private endpoint, deletion request, or playlist recreation followed.
...
An open-source adapter could add ordered reconciliation, complete pagination, durable receipts, capability reporting, and tested wrappers for verified operations. It cannot create an Apple server capability merely by exposing a new local method. The promising next research target is whether an existing replacement operation accepts our own normally authorized credentials and preserves the destination and public URL. If it requires a separate web-session integration, record that dependency explicitly before choosing it for the product.
