...
export function availablePublishers(env: PublishingSecrets): Provider[] {
  ...
  const appleDeveloperCredentials = env.APPLE_DEVELOPER_TOKEN
    || (env.APPLE_MUSIC_KEY_ID && env.APPLE_MUSIC_TEAM_ID && env.APPLE_MUSIC_PRIVATE_KEY_P8);
  if (env.APPLE_PUBLISHING_ENABLED === "true" && appleDeveloperCredentials && env.APPLE_MUSIC_USER_TOKEN
    && (!env.APPLE_MUSIC_STOREFRONT || /^[a-z]{2}$/.test(env.APPLE_MUSIC_STOREFRONT))) providers.push("apple");
  return providers;
}
