...
export function threadPage(view: ThreadView | null, managed: boolean, availableProviders: Provider[] = [], accountSubscriptions = false): string {
  ...
  return shell(view.title, `<main class="thread-shell" id="thread" data-capability="${view.publicCapability}" data-revision="${view.revision}"><div class="thread-heading"><span class="thread-eyebrow">Thread${managed ? " · You manage this" : ""}</span><h1>${escape(view.title)}</h1><p class="hero-description">${open ? "A shared collection of songs, in your order." : "This Thread is closed. You can still open its songs."}</p></div>
    ...
    ${accountSubscriptions ? `<section class="thread-sync" id="personal-subscription" data-capability="${view.publicCapability}" aria-label="Your playlist"><div class="section-caption"><span class="section-title">Your playlist</span><a data-subscription-settings class="text-button" href="/settings?thread=${view.publicCapability}">Account settings ↗</a></div><p data-subscription-status role="status" aria-live="polite">Loading your subscription…</p><a data-subscription-playlist class="text-button" hidden>Open your playlist ↗</a><div class="connection-actions"><button data-subscription-action="subscribe" class="generate-button" hidden>Subscribe</button><button data-subscription-action="retry" class="text-button" hidden>Retry sync</button><button data-subscription-action="unsubscribe" class="text-button" hidden>Unsubscribe</button></div><p class="thread-note">Subscribe to keep a personal playlist updated as songs are added.</p></section><script type="module" src="/subscription.js"></script>` : `<section class="thread-sync" aria-label="Music app sync"><div class="section-caption"><span class="section-title">Music apps</span>${managed ? `<a class="text-button" href="/t/${view.publicCapability}/manage/apps">Connect music apps ↗</a>` : ""}</div><div class="sync-providers">${view.publications.map(publication => publicationCard(publication, managed && availableProviders.includes(publication.provider))).join("")}</div><p class="thread-note">Songs and their order are saved here. Playlist links appear after the music app confirms the songs. Refresh to check sync progress.</p></section>`}
