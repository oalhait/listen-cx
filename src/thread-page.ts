...
function songRow(song: ThreadContribution, index: number, count: number, managed: boolean): string {
  ...
  const votes = `<div class="song-votes" role="group" aria-label="Vote on ${title}">
    <button class="vote-button" type="button" data-vote="up" data-id="${song.id}" aria-pressed="false" aria-label="Upvote ${title}, 0 upvotes" disabled><span aria-hidden="true">↑</span><span data-vote-count="up">0</span></button>
    <span class="vote-score" data-vote-score aria-label="Score 0">0</span>
    <button class="vote-button" type="button" data-vote="down" data-id="${song.id}" aria-pressed="false" aria-label="Downvote ${title}, 0 downvotes" disabled><span aria-hidden="true">↓</span><span data-vote-count="down">0</span></button>
  </div>`;
  return `<li class="thread-song" data-contribution-id="${song.id}">${artwork(song.artworkUrl)}<div class="track-meta"><a href="/${encodeURIComponent(song.linkSlug)}"><strong>${title}</strong></a><span>${escape(song.artist)}</span>${contributor(song)}</div>${votes}${managed ? `<div class="song-actions"><button type="button" data-move="up" data-id="${song.id}" aria-label="Move ${title} up" ${index === 0 ? "disabled" : ""}>↑</button><button type="button" data-move="down" data-id="${song.id}" aria-label="Move ${title} down" ${index === count - 1 ? "disabled" : ""}>↓</button><button type="button" data-remove="${song.id}" aria-label="Remove ${title}">Remove</button></div>` : ""}</li>`;
}
...
export function threadPage(view: ThreadView | null, managed: boolean, availableProviders: Provider[] = [], accountSubscriptions = false, baseUrl = "https://listen.cx"): string {
  ...
  const metadata = socialMetadata({
    title: `${view.title} — a music Jam`,
    description: count === 0 ? "A shared music Jam. Add the first song on listen.cx." : `${count} ${count === 1 ? "song" : "songs"}, collected together. Listen, vote, or add yours on listen.cx.`,
  ...
  const title = escape(view.title);
  return shell(view.title, `<main class="thread-shell jam-shell" id="thread" data-capability="${view.publicCapability}" data-revision="${view.revision}" data-title="${title}" data-managed="${managed}" data-open="${open}" data-reorderable="${open && !appleConnected}"><div class="thread-heading"><span class="thread-eyebrow">Jam${managed ? " · You manage this" : ""}</span><h1>${title}</h1><p class="hero-description" id="thread-description">${open ? "A shared collection of songs, votes, and conversation." : "This Thread is closed. The Jam is read-only, but its songs and conversation are still here."}</p></div>
    <div class="thread-sharing"><button id="invite-friends" class="generate-button" type="button">Invite friends <span aria-hidden="true">↗</span></button><a id="public-link" href="/t/${view.publicCapability}">Sharing link</a><button type="button" data-copy="public-link">Copy link</button><span id="share-message" role="status" aria-live="polite"></span></div>
    <div class="jam-layout"><div class="jam-queue-column">
    ...
    <p id="thread-message" class="form-message" role="status" aria-live="polite"></p><button id="refresh-thread" class="text-button" type="button" hidden>Refresh Jam ↻</button><button id="retry-management" class="text-button" type="button" hidden>Retry management access</button>
    <section class="thread-songs" aria-labelledby="song-count"><div class="section-caption"><span class="section-title" id="song-count">${view.contributions.length} ${view.contributions.length === 1 ? "song" : "songs"}</span><button id="refresh-songs" class="text-button" type="button">Refresh ↻</button></div>${managed && open && appleConnected ? '<p class="thread-note">Apple Music is connected, so songs cannot be removed or reordered. You can still add songs and close the Jam.</p>' : ""}<ol id="thread-song-list">${view.contributions.map((song, index) => songRow(song, index, view.contributions.length, managed && open && !appleConnected)).join("")}</ol><p id="thread-empty" class="thread-empty" ${view.contributions.length === 0 ? "" : "hidden"}>Start with a song you love.</p></section></div>
    <aside class="jam-collaboration" aria-labelledby="jam-chat-heading"><div class="jam-collaboration-heading"><div><span class="thread-eyebrow">Together</span><h2 id="jam-chat-heading">Jam chat</h2></div><span id="participant-count" class="participant-count">Connecting…</span></div>
      <p id="collaboration-status" class="thread-note live-status" role="status" aria-live="polite">Connecting live updates…</p><button id="retry-collaboration" class="text-button" type="button" hidden>Retry live updates</button>
      ${open ? `<form id="jam-join-form" class="jam-join" hidden><label for="jam-display-name">Join with a display name</label><div class="join-fields"><input id="jam-display-name" name="displayName" maxlength="40" required autocomplete="nickname" placeholder="Your name"><button class="generate-button" type="submit">Join Jam</button></div><p class="thread-note">Friends with this link can see your name, votes, and messages.</p><p id="join-status" class="form-message" role="status" aria-live="polite"></p></form><div id="jam-viewer" class="jam-viewer" hidden><span class="viewer-dot" aria-hidden="true"></span><span>Joining as <strong id="jam-viewer-name"></strong></span></div>` : '<p class="closed-note">Voting and chat are closed.</p>'}
      <div id="chat-log" class="chat-log" role="log" aria-live="off" aria-relevant="additions text" aria-label="Jam messages"><ol id="chat-message-list"></ol><p id="chat-empty" class="chat-empty">No messages yet. Start the conversation.</p></div>
      ${open ? '<form id="chat-form" class="chat-form" hidden><label class="sr-only" for="chat-message">Message the Jam</label><textarea id="chat-message" name="message" maxlength="500" rows="3" placeholder="Say something about the songs…" aria-describedby="chat-counter chat-status" required></textarea><div class="chat-compose-actions"><span id="chat-counter">0 / 500</span><button class="generate-button" type="submit">Send</button></div></form>' : ""}
      <p id="chat-readonly" class="thread-note" ${open ? "" : "hidden"}>Join the Jam to vote and send messages.</p><p id="chat-status" class="form-message" role="status" aria-live="polite"></p>
    </aside></div>
    ...
    ${managed && open ? '<section class="thread-management" id="thread-management"><button id="close-thread" class="text-button" type="button">Close Jam</button><div id="close-confirmation" hidden><p>Stop songs, voting, and chat in this Jam? Everything already shared will stay available.</p><button id="confirm-close" class="text-button" type="button">Close Jam now</button><button id="cancel-close" class="text-button" type="button">Keep it open</button></div></section>' : ""}</main>`, metadata);
}
